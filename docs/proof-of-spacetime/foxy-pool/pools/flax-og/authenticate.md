...
From your [My Farmer](https://flax-og.foxypool.io/my-farmer){target=_blank} page on the pool you will see a green authentication button.
...
1. Change into the directory, where the `flax` binary is located at. When using a git install this is inside the `./activate`-ed repo directory. Otherwise, please use the following:
    ...
    === "Mac OS"
        ...
        ```zsh
        cd /Applications/Flax.app/Contents/Resources/app.asar.unpacked/daemon
        ```

    === "Linux"
        Open a terminal/bash and enter the following command:
        ```bash
        cd /usr/lib/flax-blockchain/resources/app.asar.unpacked/daemon
        ```
